    ...
    (nodes as VNode[])
      .filter(v => {
        return (
          ...
          (v.componentOptions.Ctor as any).options !== undefined &&
          // 接受带label属性的column组件，label属性是必须的
          ((ParamData.__checkType(
            (v.componentOptions.Ctor as any).options.name,
      ...
      .forEach(v => {
        const type = (v.componentOptions?.Ctor as any).options.name as string;
        ...
        if (ParamData.__checkType(type, Variables.name.column)) {
          ...
          const width = parseNumber(
            (v.componentOptions?.propsData as any)?.width,
            Variables.size.defaultTableColumnWidth
          );

          // 添加 merge 方法，忽略第一行
          let mergeOps = (v.componentOptions?.propsData as any)?.merge;
          ...
          this.__addTHeader(
            new TableHeader().initData({
              key: colVNodeKey,
              label,
              text: (v.componentOptions?.propsData as any)?.name || label,
              width
            })
          );
